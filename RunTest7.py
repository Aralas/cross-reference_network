...
def initialization(file_index, lambda_weight):
    ...
    model_dirs = dirs + 'model/' + dataset + '_RunTest7_' + str(file_index) + '/'
    if not os.path.exists(model_dirs):
        os.makedirs(model_dirs)

    record_file = dirs + dataset + '_RunTest7_' + str(file_index) + '.txt'
...
def run_cross_reference(section, file_index, lambda_weight):
    ...
    set_random_seed(section)
    for label in range(num_classes):
        print('section: ', section, 'label: ', label)
        classifier = binary_classifier_list[label]
        indeces_positive = list(np.where(y_train[:, label] == 1)[0])
        indeces_negative = list(np.where(y_train[:, label] != 1)[0])
        index_train = random.sample(indeces_positive, batch_size // 2 - data_size) + random.sample(indeces_negative,
                                                                                                   batch_size // 2)
        x = np.append(clean_x[index_clean_matrix[label]], x_train[index_train], axis=0)
        y = np.array([1] * (batch_size // 2) + [0] * (batch_size // 2)).reshape(batch_size, 1)
